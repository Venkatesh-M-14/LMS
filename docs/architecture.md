...
## Social & collaboration (M10)

- **Chat — one model, three surfaces.** A single `ChatChannel` (GROUP | DIRECT | LESSON) backs the circle-wide room, 1:1 DMs, and per-lesson discussion threads. The channel `key` is a unique dedupe identity (`group`, `lesson:<id>`, `dm:<sortedIds>`), so get-or-create is a single atomic upsert rather than a read-then-write race — and a DM resolves to the same channel whichever member opens it. GROUP/LESSON are open to the whole circle (membership rows are created lazily and only carry the read marker); DIRECT membership is the ACL. Delivery targets each recipient's own authenticated Socket.IO room (`chat:message`), so a DM can only ever reach its two members — there is no per-channel room authorization to get wrong. History is cursor-paginated; unread counts come from the per-member `lastReadAt`.
- **The circle hears about each other.** New domain events (`AchievementUnlocked`, `CertificateIssued`, plus `LeaderboardOvertaken`, `SuggestionSubmitted`, `SuggestionReviewed`) drive a peer fan-out: everyone gets an in-app `PEER_SUCCESS` notification when a member passes a quiz, earns an achievement, or completes a certificate — minus the actor, and honouring a per-user opt-out. **Email is reserved for milestones**: only a certificate (a peer's or your own) sends mail, so inboxes stay usable. Preferences (`notifyPeerSuccess`, `notifyOvertaken`, `emailMilestones`) live on the user and are edited from the notification menu.
- **Overtake detection.** Each XP-awarding batch is wrapped in a rank read before/after (one Redis `ZREVRANK` each), and everyone in the passed `[after+1, before]` rank band is notified they were overtaken — if they haven't opted out. Reading rank once per batch (not per award) means one quiz that grants several XP entries produces a single, correct set of overtake events.
- **Leaderboard progress.** Entries now carry curriculum progress (lessons completed / total, current topic) alongside XP, so the circle sees how far each member actually is, not just their score.
- **Student → syllabus.** A `SyllabusSuggestion` is either a free-text IDEA or a complete DRAFT_QUESTION that reuses the *same* item contract the authoring editor writes. Students submit from any account; an **admin-only** inbox (instructors don't manage the backlog) reviews them. Accepting a draft appends it to the target lesson's question bank via a guarded, PENDING-only transition — the item is created first, so a failed append leaves the suggestion reviewable rather than lost, and two reviewers can't both win. The author is notified of the decision; admins are notified of new submissions.

## Notifications, email, analytics, PWA & hardening (M9)

- **Notifications are event-driven.** A `NotificationService` subscribes to the domain events users care about (`AttemptGraded`, `AchievementUnlocked`, `CertificateIssued`, `ProjectReviewed`, `RevisionAssigned` — the last four added in M9 and emitted from gamification/projects/adaptive) and writes `Notification` rows via pure content mappers. Each new row is pushed to the owner's Socket.IO room (`notification:new`); the web bell shows an unread badge and a dropdown, invalidating its query on push with a 60s poll fallback. The server wires the pusher after boot (`setPusher`), so the service has no direct Socket.IO dependency and stays unit-testable.
- **Email uses the transactional-outbox pattern.** Subscribers (`UserRegistered` → welcome, `CertificateIssued`, `ProjectReviewed`) write a PENDING `EmailOutbox` row and enqueue a BullMQ job — mail is never sent inside a request. A drain worker renders a template and sends over SMTP (nodemailer → mailpit in dev), flipping the row SENT, or FAILED after exhausting retries (exponential backoff). **No SMTP configured → the worker marks rows SENT without sending**, so dev/CI exercise the whole pipeline without a mail server. A boot-time `resumePending` re-enqueues rows stranded by a crash between the DB write and the enqueue, so nothing is lost or double-sent (jobId = outbox id collapses duplicates).
- **Analytics is an append-only stream.** `AnalyticsEvent` is batch-inserted from two sources: the client posts batched events to `/analytics/events` (a per-tab session id, timestamps clamped server-side against skew), and the server captures authoritative funnel events (`quiz.submitted`/`quiz.passed` from `AttemptGraded`, `lesson.opened` from the curriculum read). The instructor/admin dashboard aggregates with raw SQL (distinct active learners, daily activity, pass rate, engagement funnel, top lessons). The `occurredAt` column carries a **BRIN index** — tiny and cheap for an ever-growing, time-ordered table.
- **PWA (Workbox via vite-plugin-pwa).** A generated service worker precaches the app shell and applies runtime caching: curriculum/lesson GETs are **NetworkFirst** (readable offline after a first view), while everything under `/api/v1/assessments` is **NetworkOnly** — grading and attempts must never be served from cache. A web-app manifest makes it installable; an unobtrusive prompt offers to reload on a new SW version, and an offline banner appears when the network drops.
- **WCAG 2.2 AA gate.** `@axe-core/playwright` scans representative pages (auth, dashboard, curriculum, mentor) at the `wcag2a`/`wcag2aa`/`wcag21aa`/`wcag22aa` tags in CI and fails on any violation.
- **Load-tested hot paths.** `pnpm load` (autocannon) drives the Redis-backed leaderboard read and the analytics batch-ingest write; on a dev laptop both sustain ~2,200 req/s at p99 < 30ms with zero errors (run with raised rate limits — see [testing.md](./testing.md)).

## AI Mentor & adaptive learning (M8)

- **Provider port, swappable adapters.** `MentorService` depends only on the `LlmProvider` port (`isConfigured()` + `streamReply(system, history) → { stream, usage }`). Three implementations: the real [`AnthropicProvider`](../apps/api/src/modules/mentor/infrastructure/anthropicProvider.ts) (`@anthropic-ai/sdk`, `messages.stream`), a deterministic [`FakeMentorProvider`](../apps/api/src/modules/mentor/infrastructure/fakeProvider.ts) for dev/CI (no key, no network — every mentor test and the E2E run against it), and an `UnconfiguredMentorProvider` that reports itself unconfigured so the API degrades cleanly (`MENTOR_NOT_CONFIGURED`) instead of crashing. `MENTOR_PROVIDER` + `ANTHROPIC_API_KEY` select at composition time.
- **SSE streaming.** The send endpoint is an async generator yielding a typed `MentorStreamChunk` union (`start → delta* → done | error`), written as `data: <json>\n\n` frames with streaming headers (`text/event-stream`, `X-Accel-Buffering: no`). The web client reads the stream with `fetch` (not `EventSource`, so it can POST + send the bearer token) and appends deltas to a live bubble; the budget bar updates from the `done` frame.
- **Grounding never sees answers.** A lesson-anchored conversation grounds the system prompt in the lesson's **currently-published** content blocks only (MARKDOWN/CALLOUT/CODE, capped to ~4 000 chars). Hidden test cases, quiz answer keys, and solutions live in relational tables that grounding never queries, and the persona itself disclaims access to them. This is verified by a unit test asserting the assembled system prompt carries only the published excerpt.
- **Per-user daily token budget.** `MentorUsage` (composite PK `userId + day`, `day` = the user's local `YYYY-MM-DD`) accumulates input+output tokens; the budget is checked up front (`MENTOR_BUDGET_EXCEEDED` before any model call) and incremented after the stream completes. History is trimmed to the last N turns so context and cost stay bounded.
- **Adaptive weakness → revision → retake gate.** On a failed `AttemptGraded`, pure [`detectWeakSkills`](../apps/api/src/modules/adaptive/domain/weakness.ts) maps every missed skill-tagged item to a weak skill (skills read from live items, since attempt snapshots don't carry tags). Each weak skill yields an idempotent `RevisionAssignment` (unique `userId + assessmentId + skillId`) **targeting the lesson the learner just failed** — always accessible, avoiding the locked-remediation-lesson trap. An open blocking assignment gates the retake (`assertRetakeAllowed` throws `REVISION_REQUIRED` 409 from the attempt-start path); opening the target lesson completes the assignment (`onLessonOpened`), which unblocks the retake. A lesson view refetches on every mount so the server-side completion always fires even on a client-cached re-visit.
- **Wiring**: `AdaptiveService` subscribes to `AttemptGraded` and is injected into the attempt service as the retake gate and into the curriculum router as the lesson-open callback — no direct dependency between assessments, curriculum, and adaptive.
